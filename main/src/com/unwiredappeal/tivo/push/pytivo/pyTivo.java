...
import com.unwiredappeal.tivo.push.PushHandler;
import com.unwiredappeal.tivo.push.Tivo;
import com.unwiredappeal.tivo.utils.Log;

public class pyTivo implements PushHandler {
   ...
   public List<Tivo> getTivos() {
	  List<Tivo> tlist = new ArrayList<Tivo>();
	  if (tivos != null) {
		  for (int i=0;i<tivos.size();i++)
			  tlist.add(new Tivo(tivos.get(i)));
	  }
      return tlist;
   }
   ...
   public static void setup() {
	...
	public boolean canPush(DirEntry de, int qual) {
		String uri = de.getUri().toString();
		video video = findVideo(uri.toString());
		return video != null && getTivos().size() > 0;
	}
	
	public boolean pushVideo(URL baseUri, DirEntry de, Tivo tivo, int qual) {
		video video = findVideo(de.getUri().toString());
		if (video != null) {
			return pushVideo(video, tivo.getName());
		}
		return false;
	}
}
